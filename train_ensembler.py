...
def get_scores(X, y):
    ...
    del(X, y_pred_1, y_pred_2, y_pred_3, y_pred_4, y_pred_5,
        y_pred_6, y_pred_7, y_pred_8, y_pred_9, y_pred_10)
...
def train_step(image, target):
    with tf.GradientTape() as ens_tape:
        ...
        dice_necrotic = diceNecrotic(target, ens_output)
        dice_edema = diceEdema(target, ens_output)
        dice_enhancing = diceEnhancing(target, ens_output)
        print('train_step:  dice_necrotic, dice_edema, dice_enhancing')
        print(dice_necrotic, dice_edema, dice_enhancing)

    ensembler_gradients = ens_tape.gradient(dice_loss, E.trainable_variables)
    ensembler_optimizer.apply_gradients(
        zip(ensembler_gradients, E.trainable_variables))
